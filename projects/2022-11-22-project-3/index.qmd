...
description: "Building websites for R packages"
date: 2022-11-29
draft: true
...
3. Your analysis must include some form of data wrangling and data visualization. 
  ...
  - You must use at least two functions from `purr`. 
...
If the package does not already include one, make and include a `README.md` file in the folder where the R package and pkgdown files are on your computer and add the following information below. 
...
You need to modify the template code that is provided to you from GitHub when you set the remote. There will already be a remote `origin` (from where you cloned the repo), which you can see with 

``` {.bash filename="Bash"}
git remote -v
```

To change where you push your code, instead of 
...
and when you push, you use `git push -u upstream main`, for example.
