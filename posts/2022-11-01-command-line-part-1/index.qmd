...
-   Be able to create, copy, move, rename, and delete files and folders
-   Be able to print and sort file contents
-   Be able to search for regular expressions in files
-   Be able to execute R commands and scripts in the command line
:::
...
- If you are working on a Mac, Apple calls the shell 'Terminal'. There is an application you can open called 'Terminal' and it also appears in a tab next to the R console in the RStudio IDE. 
...
| Command	| Description |  
| ------- | ------------| 
| `nameofcommand -h` | For some `nameofcommand` command (e.g. `whoami`)
| `man nameofcommand`   | Display manual pages for the `nameofcommand` command | 
...
For most commands, `NAMEOFCOMMAND -h` or `NAMEOFCOMMAND --help` will bring up a small guide to command options.
...
If we wanted to redirect that output from printing to the terminal and write to a file, we use the `>` operator.
...
An important feature of the command line is the PATH variable. I won't get into all the details about the PATH variable, but having a basic understanding will likely prove useful if you ever have to troubleshoot problems in the future.
...
The answer is that your system has a list of folders stored in an “environment variable” called PATH, and when you run a command (like `python` or `ls`), it goes through those folders in order until it finds an executable file with the name of the command you typed. Then, when it finds that file, it executes that program and stops looking.
...
That means that when I type `python`, my computer will first look in the folder `/opt/homebrew/bin` to see if there is a file named `python` it can run. If it can’t find one there, it moves on to `/opt/homebrew/sbin:`, etc.
...
- If you downloaded a program, but you can not run it from the command line, that probably means that its location is not in the PATH variable.
- If you find that when you type a command like `python`, the command line isn not running the version of python you want it to run, that’s probably because a different version of python appears earlier in the PATH variable (since the command line will stop looking through these folders as soon as it finds a match). 

Note that you can diagnose this problem by typing `which COMMANDNAME`, which will tell you the folder from which `COMMANDNAME` is being run.
...
The `cat` (stands for **cat**enate) command reads data from files, and outputs their contents to the screen. 
...
`R -e` will launch R and then execute the code specified within quotes. Use semi-colon to execute multiple expressions as shown below. You will be able to run the below commands only if you are able to launch R from the command line. Windows users need to ensure that R is added to the path environment.
...
2.  Practice your command line knowledge with [Command Challenge](https://cmdchallenge.com/).
:::
